...
import { Brain, Zap, Target, Cog, Eye, Plus, Check } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useToast } from "@/hooks/use-toast";

interface ModelSelectorButtonsProps {
  selectedModel: string;
  setSelectedModel: (model: string) => void;
}

const ModelSelectorButtons = ({ selectedModel, setSelectedModel }: ModelSelectorButtonsProps) => {
            ...
            {githubRepos.map((repo) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {selectedRepo === repo.id && <Check className="w-4 h-4" />}
                </div>
              </button>
            ))}
          </div>
...
export default ModelSelectorButtons;
